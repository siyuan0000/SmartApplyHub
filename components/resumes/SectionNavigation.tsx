...
export function SectionNavigation({ onSectionEdit }: SectionNavigationProps) {
  ...
  // Function to check section completion status
  const getSectionCompletionStatus = (section: SectionType): 'complete' | 'incomplete' | 'empty' => {
    if (!content) return 'empty'
    
    switch (section) {
      case 'contact':
        return content.contact?.name ? 'complete' : 'empty'
      case 'summary':
        return content.summary ? 'complete' : 'empty'
      case 'experience':
        return content.experience?.length > 0 ? 'complete' : 'empty'
      case 'education':
        return content.education?.length > 0 ? 'complete' : 'empty'
      case 'skills':
        return content.skills?.length > 0 ? 'complete' : 'empty'
      case 'projects':
        return (content.projects?.length ?? 0) > 0 ? 'complete' : 'empty'
      default:
        return 'empty'
    }
  }
